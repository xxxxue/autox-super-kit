...
export function init_web_view_ui(p_data: string, p_data_type: "file-path" | "web-url" | "html-string") {
  ...
  if (p_data_type == "web-url") {
    ...
    // 本地文件
    let path = "file:" + files.path(p_data); // 相对路径 转 绝对路径
    ui.web.loadUrl(path);
  }
}
/**
...
export async function call_autojs_fn(p_function_name: string, p_args: any | any[]) {
  ...
  let fn: Function = globalThis[p_function_name];
  if (!fn) {
    throw new Error("call_auto_js: " + p_function_name + " 没有定义实现");
  }

  // 判断是否为 async 函数
  let is_async_fn = typeof fn === "function" && Object.prototype.toString.call(fn) === "[object AsyncFunction]";
  ...
  if (Array.isArray(p_args)) {
    if (is_async_fn) {
      ...
      ret = await new Promise<any>((resolve, reject) => {
        try {
          let r = fn.apply(globalThis, p_args);
          resolve(r);
        } catch (error) {
          reject(error);
        }
    ...
    if (is_async_fn) {
      ...
      ret = await new Promise<any>((resolve, reject) => {
        try {
          let r = fn.call(globalThis, p_args);
          resolve(r);
        } catch (error) {
          reject(error);
        }
...
function init_web_view(p_web_view_widget, p_support_zoom = false) {
  var settings = p_web_view_widget.getSettings();
  ...
  p_web_view_widget.webChromeClient = new JavaAdapter(android.webkit.WebChromeClient, {
    ...
    onConsoleMessage: async (consoleMessage) => {
      let msg: string = consoleMessage.message();
      ...
      if (msg.indexOf("jsbridge://") == 0) {
        ...
        let data: IWebBridge = JSON.parse(java.net.URLDecoder.decode(uris[2], "UTF-8"));
        let cmd = data.cmd;
