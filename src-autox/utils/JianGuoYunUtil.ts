...
export class JianGuoYunUtil {
   ...
   */
  public setThisFileName(fileName, fileExtension) {
  ...
  public getFolderCatalog() {
    let httpResp = http.request(this.jgyHost, {
      method: "PROPFIND",
      headers: this.header,
    });
    let retArr: string[] = [];
    let xmlData = httpResp.body.string();
    if (xmlData) {
  ...
  public delete(fileName) {
    let ret = { isSuccess: false, msg: "删除失败" };
  ...
  public putText(data: string, fileName?: string) {
    let ret = { isSuccess: false, msg: "上传失败" };
    try {
      fileName = this.__getThisFileName(fileName);
      let httpResp = http.request(this.jgyHost + fileName, {
        method: "PUT",
  ...
  public downLoadFile(isByteData?: boolean, fileName?: string) {
    let ret = { isSuccess: false, msg: "获取失败" };
    try {
      fileName = this.__getThisFileName(fileName);
      let httpResp = http.get(this.jgyHost + fileName, {
        headers: this.header,
      });
      if (httpResp.statusCode == 404) {
        let strRes = httpResp.body.string();
  ...
  public putFile(byteData: any[], fileName?: string) {
    ...
    try {
      ...
      // 所以这里自己调用 okhttp
      let client = new MutableOkHttp()
      let mediaType = MediaType.parse("application/octet-stream");
      let requestBody = RequestBody.create(mediaType, byteData);
