...
export class SQLiteUtil {
   ...
   */
  insert(tableName: string, jsonObj: object, nullColumnHack?: string) {
    this._log("insert");
    let res = -1;
    let db: any;
   ...
   */
  delete(tableName: string, whereClause: string, whereArgs: any[]) {
    this._log("delete");
    let res = 0;
    let db: any;
   ...
   */
  update(tableName: string, jsonObj: object, whereClause: string, whereArgs: any[]) {
    this._log("update");
    let res = 0;
    let db: any;
   ...
   * @param {string?} nullColumnHack 一般为 null 即可   https://www.iteye.com/blog/mofan-1412262
   * @returns {number} 新插入的行的行 ID；如果发生错误，则返回 -1
   */
  replace(tableName: string, jsonObj: object, nullColumnHack?: string) {
   ...
   * @returns {boolean} 如果新版本代码大于当前数据库版本，则返回 true。
   */
  needUpgrade(newVersion: number) {
    let res = false;
    let db: any;
   ...
   * @returns {object[]} json 数组
   */
  private _getCursorDataArr(cursor: any) {
    const res: object[] = [];
