...
export let runRollup = async () => {
    ...
    let code_file_name = out.output[0].fileName;
    ...
    if (_is_prod) {
        ...
        let html_path = path.join(_out_aj_dir_path, "index.html");
...
function copy_aj_config_to_out() {
    let config_file_name = "project.json"
    let file_path = path.join(process.cwd(), config_file_name);
    let target_path = path.join(_out_aj_dir_path, config_file_name);
