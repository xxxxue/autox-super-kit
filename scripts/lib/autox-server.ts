...
export class Extension {
  ...
  constructor() {
    ...
    this.autojs_server
      ...
      .on("cmd", (cmd: string, url: string) => {
        //  好像是 autox 作者自己写的 webpack 工程化项目，会用到这个 cmd 命令。
      });
  }
  ...
  showQrCode() {
    ...
    for (const item of ip_arr) {
      this.__showQrcodeToTerminal(item);
    }
  }

  private __showQrcodeToTerminal(ip: string) {
    const wsUrl = `ws://${ip}:${_autojs_server_port}`;
