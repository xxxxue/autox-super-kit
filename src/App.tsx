 ...
 * @param data 参数 / "[code]" (一个参数可以直接传, 多个参数使用数组,例如: [ 1,"hh",false ] )
 * @returns 使用 await 获取返回值
 */
export function invokeFn<T = any>(aj_fn_name: string, data: any | any[] = undefined) {
...
function App() {
  ...
  let aj_fn_1 = async () => {
    let r = await invokeFn("aj_fn_1");
    setFn_1(r);
  };
  let aj_fn_2 = async () => {
    let r = await invokeFn("aj_fn_2");
    setFn_2(r);
  };
  let aj_fn_2_1 = async () => {
    let r = await invokeFn("aj_fn_2", "小明");
    setFn_2_1(r);
  };
  let aj_fn_3 = async () => {
    let r = await invokeFn("aj_fn_3");
    setFn_3(r);
  };
  let aj_fn_4 = async () => {
    let r = await invokeFn("aj_fn_4", [111, "小明111", 555]);
    setFn_4(r);
  };
  let aj_fn_5 = async () => {
    let r = await invokeFn("aj_fn_5", {
      id: 999,
      name: "小明999",
    });
    setFn_5(r);
  ...
  return (
    ...
    >
      ...
      <button onClick={aj_fn_1}>aj_fn_1 --- {fn_1}</button>
      <button onClick={aj_fn_2}>aj_fn_2 --- {fn_2 ? "成功" : "false"}</button>
      <button onClick={aj_fn_2_1}>aj_fn_2_1 --- {fn_2_1 ? "成功" : "false"}</button>
      <button onClick={aj_fn_3}>aj_fn_3 --- {fn_3}</button>
      <button onClick={aj_fn_4}>aj_fn_4 --- {fn_4.length}</button>
      <button onClick={aj_fn_5}>aj_fn_5 --- {fn_5 && "成功"}</button>
      <button onClick={exec_code_fn}>执行 aj 代码 --- {exec_code}</button>
      <button onClick={exec_code_2_fn}>执行 aj 代码2 --- {exec_code_2}</button>
